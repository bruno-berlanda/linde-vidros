...
<?php
	...
	$consulta = mysqli_query ($conexao, "SELECT * FROM insulado_orcamentos WHERE codigo='$codigo'") or die (mysqli_error());
		$dados = mysqli_fetch_array ($consulta);
			...
			$consulta_requerente = mysqli_query ($conexao, "SELECT razao FROM clientes WHERE id='$vendedor'") or die (mysqli_error());
				$dados = mysqli_fetch_array ($consulta_requerente);
					$nome_requerente = $dados['razao'];
	...
	<div class="col-md-3">
		...
		<div class="well">
			<form method="post" action="funcoes/insulado.php?funcao=editar_cliente&orcamento=<?php echo $codigo; ?>">
				<fieldset<?php if ($vendedor != $idUsuario) { echo " disabled"; } ?>>
	...
	<div class="col-md-9">
		...
		<div class="tab-content">
			...
			<div role="tabpanel" class="tab-pane active" id="medidas">
				<div class="row">
					<div class="col-md-9">
						<form method="post" action="funcoes/insulado.php?funcao=pecas&orcamento=<?php echo $codigo; ?>" class="form-inline">
							<fieldset<?php if ($bloqueio_composicao == true || $vendedor != $idUsuario) { echo " disabled"; } ?>>
				...
				<div class="row">
					<div class="col-md-4">
						<?php if ($gas == "S" && $vendedor == $idUsuario) { // Gás = S e Usuário Logado ?>
						...
						<?php } elseif ($gas == "N" && $vendedor == $idUsuario) { // Gás = N e Usuário Logado ?>
						...
						<?php } elseif ($gas == "S" && $vendedor != $idUsuario) { // Gás = S e Usuário Não Logado ?>
						<button class="btn btn-block btn-warning disabled"><i class="fas fa-leaf"></i> Gás Argônio</a></button>
						<?php } elseif ($gas == "N" && $vendedor != $idUsuario) { // Gás = N e Usuário Não Logado ?>
					...
					<div class="col-md-4">
						...
						<?php if ($silicone_t == "S" && $vendedor == $idUsuario) { // Silicone = S e Usuário Logado ?>
						...
						<?php } elseif ($silicone_t == "N" && $vendedor == $idUsuario) { // Silicone = N e Usuário Logado ?>
						...
						<?php } elseif ($silicone_t == "S" && $vendedor != $idUsuario) { // Silicone = S e Usuário Não Logado ?>
						<button class="btn btn-block btn-warning disabled"><i class="fas fa-retweet"></i> Silicone em todas as peças</a></button>
						<?php } elseif ($silicone_t == "N" && $vendedor != $idUsuario) { // Silicone = N e Usuário Não Logado ?>
					...
					<div class="col-md-4">
						<?php if ($t_camaras == "DUR" || $t_camaras == "DLT") { ?>
						<?php if ($silicone_c == "S" && $vendedor == $idUsuario) { // Silicone = S e Usuário Logado ?>
						...
						<?php } elseif ($silicone_c == "N" && $vendedor == $idUsuario) { // Silicone = N e Usuário Logado ?>
						...
						<?php } elseif ($silicone_c == "S" && $vendedor != $idUsuario) { // Silicone = S e Usuário Não Logado ?>
						<button class="btn btn-block btn-warning disabled"><i class="fas fa-expand-arrows-alt"></i> Silicone nos cantos</a></button>
						<?php } elseif ($silicone_c == "N" && $vendedor != $idUsuario) { // Silicone = N e Usuário Não Logado ?>
