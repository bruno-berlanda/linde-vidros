...
<?php
$consulta_orcamentos = mysqli_query ($conexao, "SELECT * FROM insulado_orcamentos WHERE usuario='$idUsuario' AND tipo_usuario='C' ORDER BY id DESC") or die (mysqli_error());
...
while ($linha = mysqli_fetch_array($consulta_orcamentos)) {
	...
	$consulta_requerente = mysqli_query ($conexao, "SELECT razao FROM clientes WHERE id='$usuario'") or die (mysqli_error());
