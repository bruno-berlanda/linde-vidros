...
if ($cliente == "") {
	...
	$codigo = md5(date("YmdHis").$cliente.$idUsuario);
	...
	$cadastra = mysqli_query ($conexao, "INSERT INTO insulado_orcamentos (codigo, usuario, ip, tipo_usuario, cliente_nome, neg_data, neg_usuario, neg_ip, neg_status, neg_limite) VALUES ('$codigo', '$idUsuario', '$ip', 'C', '$cliente', '$data_agora', '$idUsuario', '$ip', 'G', '$data_negociacao')") or die (mysqli_error());
	
	// Pega o ID do orçamento solicitado
	$consulta_cadastro = mysqli_query ($conexao, "SELECT id FROM insulado_orcamentos WHERE usuario='$idUsuario' AND ip='$ip' AND cliente_nome='$cliente' ORDER BY id DESC") or die (mysqli_error());
