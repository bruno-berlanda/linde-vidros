<form method="get" action="moveleiro-gerenciar-pedidos.php" class="form-horizontal">
    <fieldset>
        ...
        <div class="form-group form-group-sm">
            <label for="selectRequerente" class="col-sm-3 control-label">Vendedor</label>
            <div class="col-sm-9">
                <select name="filtro_requerente" id="selectRequerente" class="form-control">
                    <option></option>
                    <?php
                    $consulta_usuarios = mysqli_query ($conexao, "SELECT a.usuario, b.id, b.nome FROM moveleiro_pedidos a, representantes b WHERE a.usuario=b.id GROUP BY a.usuario ORDER BY b.nome") or die (mysqli_error());
                        while ($dados = mysqli_fetch_array($consulta_usuarios)) {
                            $idRequerente	= $dados['usuario'];
                            $nomeRequerente	= $dados['nome'];
                    ?>
                    <option value="<?php echo $idRequerente; ?>"<?php if (isset($_GET['filtro_requerente']) && $_GET['filtro_requerente'] == $idRequerente) { echo " selected"; } ?>><?php echo nome_sobrenome($nomeRequerente); ?></option>
                    <?php
                        }
                    ?>
                </select>
