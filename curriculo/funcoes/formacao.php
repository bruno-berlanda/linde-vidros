...
$atividade 		= caracteres(strip_tags(trim($_POST['atividade'])));
$carga 			= strip_tags(trim($_POST['carga']));
...
$atividade 		= strtr(strtoupper($atividade),"àáâãäåæçèéêëìíîïðñòóôõö÷øùüúþÿ","ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÜÚÞß");
...
$cadastra = mysqli_query ($conexao, "INSERT INTO usuarios_formacao (id_usuario, ini_mes, ini_ano, con_mes, con_ano, instituicao, atividade, carga, situacao) VALUES ('$id_usuario', '$ini_mes', '$ini_ano', '$con_mes', '$con_ano', '$instituicao', '$atividade', '$carga', '$situacao')") or die (mysqli_error());
$atualiza = mysqli_query ($conexao, "UPDATE usuarios SET atualizado='$data', lido='0', ativo='1' WHERE codigo='$c_usuario'") or die (mysqli_error());
	...
	header ('Location: ../formacao.php?msgSucesso=Formação cadastrada com sucesso');
...
$dt_conc = explode ("/", $data_conclusao);
	...
	$atualiza1 = mysqli_query ($conexao, "UPDATE usuarios_formacao SET ini_mes='$ini_mes', ini_ano='$ini_ano', con_mes='$con_mes', con_ano='$con_ano', instituicao='$instituicao', atividade='$atividade', carga='$carga', situacao='$situacao' WHERE id='$id'") or die (mysqli_error());
	...
	header ('Location: ../formacao.php?msgSucesso=Formação alterada com sucesso');
...
$verifica_id = mysqli_query ($conexao, "SELECT id_usuario FROM usuarios_formacao WHERE id=$id") or die (mysqli_error());
	$dados = mysqli_fetch_array ($verifica_id);
		...
		if ($id_usuario != $idUsuario_esc) {
			...
			$exclui = mysqli_query ($conexao, "DELETE FROM usuarios_formacao WHERE id='$id'") or die (mysqli_error());

			$atualiza = mysqli_query ($conexao, "UPDATE usuarios SET atualizado='$data' WHERE codigo='$c_usuario'") or die (mysqli_error());
			header ('Location: ../formacao.php?msgSucesso=Formação excluída com sucesso');
		}
}