...
// Dados do formulário
$software 	= strip_tags(trim($_POST['software']));
$nivel		= strip_tags(trim($_POST['nivel']));

$cadastra = mysqli_query ($conexao, "INSERT INTO usuarios_informatica (id_usuario, software, nivel) VALUES ('$id_usuario', '$software', '$nivel')") or die (mysqli_error());
$atualiza = mysqli_query ($conexao, "UPDATE usuarios SET atualizado='$data', lido='0', ativo='1' WHERE codigo='$c_usuario'") or die (mysqli_error());
	...
	header ('Location: ../informatica.php?msgSucesso=Conhecimento cadastrado com sucesso');
...
$verifica_id = mysqli_query ($conexao, "SELECT id_usuario FROM usuarios_informatica WHERE id=$id") or die (mysqli_error());
	$dados = mysqli_fetch_array ($verifica_id);
		...
		if ($id_usuario != $idUsuario_esc) {
			header ('Location: ../informatica.php?msgErro=Você não tem permissão para acessar essa página');
		}
		else {

			$exclui = mysqli_query ($conexao, "DELETE FROM usuarios_informatica WHERE id='$id'") or die (mysqli_error());

			$atualiza = mysqli_query ($conexao, "UPDATE usuarios SET atualizado='$data' WHERE codigo='$c_usuario'") or die (mysqli_error());
			header ('Location: ../informatica.php?msgSucesso=Conhecimento excluído com sucesso');
		}
}